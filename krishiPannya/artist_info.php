...
<body>
    ...
    $user_id = $_GET['id'];
        ...
        while($row=mysqli_fetch_array($result_category)){
            ...
            echo ' <h2 class="headspace"> '.$row['user_fname'].'\'এর তথ্য <hr class="hr" style="border-bottom: 2px solid #2d70d5;"></h2>

                  <p></p>

                  <div class="backgd"><a  href= "pictures/profile/'.$row['user_imagepath'].'"> <img class="photo1" src="pictures/profile/'.$row['user_imagepath'].'"></a>

                  <p class = "info-title">নাম: <br>লোকেশন: <br>যোগাযোগ: </p></div>

                  <p class = "info-content">'.$row['user_fname'].' '.$row['user_mname'].'. '.$row['user_lname'].'<br>'.$row['user_province'].'<br>0'.$row['user_contact'].'</p></div> ';
        ...
        while ($row2=mysqli_fetch_array($result_category2)) {
                    echo '<h2 class="headspace2"> '.$row2['user_fname'].'\'এর পণ্যসমূহ<hr class="hr" style="border-bottom: 2px solid #2d70d5;"></h2>';
        }
...
if(mysqli_num_rows($result_category1) <=0)
        {
            echo '<br><br><br><br><br><br><br><h1 align="Center">এখন পর্যন্ত নেই</h1>';
        }
        else{
